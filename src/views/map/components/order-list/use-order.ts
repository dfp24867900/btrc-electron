...
import { getTimeEvent_new } from '@/service/modules/animation'
...
export function useTimeEvent(taskId: string) {
  ...
  const timeEvent = async () => {
    await getTimeEvent_new({ taskId: taskId }).then((res: TimeEvent[]) => {
  ...
  const findEvents = _.throttle((time: number) => {
    let minTime = time - 3 * 60 * 1000
    let arr = _.filter(variables.eventList, (o: TimeEvent) => {
      return o.timeVal <= time && o.timeVal >= minTime
    })
